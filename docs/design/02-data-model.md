...
- **`require:` / `set:` per transition** — a transition target may be written as a map instead of a bare state key, carrying guards:
  ...
  - `require: [blockers_closed]` — reserved token (not a field): every item in the moving item's `blocked_by` must be in a `done`-category state, or the move fails with exit 1 (`--force` downgrades to a warning). A `blocked_by` reference that resolves to no item, or to an item whose type/state carries no known category, warns and passes rather than bricking the move on missing data.
