...
func (c *compiler) compilePred(n *predExpr) (Predicate, error) {
	...
	case fieldReporter:
		return scalarPred(eq, want, func(it *item.Item) string { return deref(it.Reporter) }), nil
	case fieldSubtype:
		return scalarPred(eq, want, func(it *item.Item) string { return deref(it.Subtype) }), nil
	case fieldResolution:
		return scalarPred(eq, want, func(it *item.Item) string { return deref(it.Resolution) }), nil
	case fieldRank:
		return scalarPred(eq, want, func(it *item.Item) string { return deref(it.Rank) }), nil
	case fieldSprint:
		if n.value == "active" {
			if c.opts.ActiveSprint == "" {
				c.note(NoActiveSprintNote)
				return func(*item.Item, *config.Config) bool { return !eq }, nil
			}
			want = c.opts.ActiveSprint
		}
		return scalarPred(eq, want, func(it *item.Item) string { return deref(it.Sprint) }), nil
	case fieldCategory:
	...
	case fieldDue:
		return datePred(n, func(it *item.Item) string { return deref(it.Due) }), nil
	default:
...
func compileIsEmpty(n *emptyExpr) Predicate {
	var isEmpty func(it *item.Item, cfg *config.Config) bool
	switch n.field {
	case fieldLabel:
		isEmpty = func(it *item.Item, _ *config.Config) bool { return len(it.Labels) == 0 }
	case fieldBlockedBy:
		isEmpty = func(it *item.Item, _ *config.Config) bool { return len(it.BlockedBy) == 0 }
	case fieldLinks:
		isEmpty = func(it *item.Item, _ *config.Config) bool { return !anyLinkPresent(it.Links) }
	case fieldEstimate:
		isEmpty = func(it *item.Item, _ *config.Config) bool { return it.Estimate == nil }
	default:
		get := scalarGet(n.field)
		isEmpty = func(it *item.Item, cfg *config.Config) bool { return get(it, cfg) == "" }
	}
	want := !n.notEmpty
	return func(it *item.Item, cfg *config.Config) bool { return isEmpty(it, cfg) == want }
}

func scalarGet(field string) func(*item.Item, *config.Config) string {
	switch field {
	case fieldState:
		return func(it *item.Item, _ *config.Config) string { return it.State }
	case fieldType:
		return func(it *item.Item, _ *config.Config) string { return it.Type }
	case fieldCategory:
		return func(it *item.Item, cfg *config.Config) string { return categoryOf(cfg, it.Type, it.State) }
	case fieldOwner:
		return func(it *item.Item, _ *config.Config) string { return deref(it.Owner) }
	case fieldReporter:
		return func(it *item.Item, _ *config.Config) string { return deref(it.Reporter) }
	case fieldSubtype:
		return func(it *item.Item, _ *config.Config) string { return deref(it.Subtype) }
	case fieldResolution:
		return func(it *item.Item, _ *config.Config) string { return deref(it.Resolution) }
	case fieldPriority:
		return func(it *item.Item, _ *config.Config) string { return deref(it.Priority) }
	case fieldRank:
		return func(it *item.Item, _ *config.Config) string { return deref(it.Rank) }
	case fieldSprint:
		return func(it *item.Item, _ *config.Config) string { return deref(it.Sprint) }
	case fieldDue:
		return func(it *item.Item, _ *config.Config) string { return deref(it.Due) }
	case fieldEpic:
		return func(it *item.Item, _ *config.Config) string { return deref(it.Epic) }
	case fieldCreated:
		return func(it *item.Item, _ *config.Config) string { return it.Created }
	case fieldUpdated:
		return func(it *item.Item, _ *config.Config) string { return it.Updated }
	default:
		return func(*item.Item, *config.Config) string { return "" }
	}
}
...
func (c *compiler) rankedPriorityPred(n *predExpr) (Predicate, error) {
	...
	op := n.op.kind
	return func(it *item.Item, _ *config.Config) bool {
		got, ok := index[deref(it.Priority)]
