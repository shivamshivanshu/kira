...
var s15 = Sprint{Key: "2026-S15", Name: "Sprint 15", Start: "2026-07-27", End: "2026-08-09"}
var s16 = Sprint{Key: "2026-S16", Name: "Sprint 16", Start: "2026-08-10", End: "2026-08-23"}

func appendSprint(t *testing.T, data string, sp Sprint) string {
	t.Helper()
	out, err := AppendSprint([]byte(data), sp)
	if err != nil {
...
func TestAppendSprintDuplicateKeyRejected(t *testing.T) {
	first := appendSprint(t, commentedConfig, s15)
	if _, err := AppendSprint([]byte(first), s15); err == nil || !strings.Contains(err.Error(), "duplicate key") {
		t.Errorf("duplicate append error = %v, want duplicate key", err)
	}
}

func TestAppendSprintEmptyNameRejected(t *testing.T) {
	sp := Sprint{Key: "K", Start: "2026-01-01", End: "2026-01-02"}
	if _, err := AppendSprint([]byte(commentedConfig), sp); err == nil || !strings.Contains(err.Error(), "empty name") {
		t.Errorf("unnamed sprint error = %v, want empty name", err)
	}
}

func TestAppendSprintMultiLineValueRejected(t *testing.T) {
	sp := Sprint{Key: "K", Name: "a\nb", Start: "2026-01-01", End: "2026-01-02"}
	if _, err := AppendSprint([]byte(commentedConfig), sp); err == nil {
