...
// AppendSprint splices only the sprints block: a whole-document yaml re-encode
// would normalize comment alignment and flow styles across the hand-edited
// config.yaml, so every byte outside the block must survive unchanged. The
// result is re-parsed before being returned, so a splice bug can never
// produce an unloadable config.
func AppendSprint(data []byte, sp Sprint) ([]byte, error) {
...
func inlineSprintEntry(sp Sprint) (string, error) {
	fields := [4]string{}
