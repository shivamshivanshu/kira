...
// documented default (docs/design/02-data-model.md §9) with deliberate
// differences: the project key/name are the caller's, and labels.known /
// people.known / filters / sprints are seeded empty (a fresh project has no
// vocabulary, saved queries, or sprints yet — the doc example's entries are
// illustrations, not defaults). Every omitted field inherits config.Default()
// via config.Parse, so this stays the human-editable surface, not the
// exhaustive schema. Pinned by an init unit test that round-trips it through
// config.Parse.
