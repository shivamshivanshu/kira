...
func (s *Store) stateEvents(ulid string) ([]transitionEvent, error) {
	rel := filepath.Join(dirName, "tickets", ulid+".md")
	out, err := s.git("log", "--follow", "--format=%x01%cI", "-p", "--", rel)
	if err != nil {
	...
	for _, line := range strings.Split(out, "\n") {
		...
		case strings.HasPrefix(line, "\x01"):
			...
			if perr != nil {
				return nil, userErr("parsing commit date %q: %v", line[1:], perr)
			}
			ts = t
...
func (s *Store) itemDoneInfo(cfg *config.Config, it *item.Item) (doneInfo, error) {
...
func isDoneState(cfg *config.Config, typ, state string) bool {
	cat, ok := categoryOf(cfg, typ, state)
	return ok && cat == config.CategoryDone
}
