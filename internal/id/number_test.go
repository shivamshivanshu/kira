...
func TestAllocateEmptyStartsAtOne(t *testing.T) {
	if got := Allocate(Snapshot{Key: "KIRA"}); got.N != 1 || got.String() != "KIRA-1" {
...
func TestAllocateIsMaxUnionPlusOne(t *testing.T) {
	...
	for trial := 0; trial < 500; trial++ {
		...
		got := Allocate(Snapshot{Key: "KIRA", Items: items})
		want := max + 1
		if got.N != want {
...
func TestAllocateCountsAliasesAndIgnoresForeignKeys(t *testing.T) {
	snap := Snapshot{Key: "KIRA", Items: []Item{
		{Number: "KIRA-3"},
		{Number: "kira-5"}, // case-insensitive key still counts
		{Number: "KIRA-2", Aliases: []string{"KIRA-40"}}, // alias dominates the max
		{Number: "OTHER-9000"},                           // foreign key ignored
		{Number: "not-a-number"},                         // unparseable ignored
	}}
	if got := Allocate(snap); got.N != 41 {
...
func TestHashNumberDerivesFromULID(t *testing.T) {
	u := Mint()
	got := HashNumber("KIRA", u)
	if got != HashNumber("KIRA", u) {
